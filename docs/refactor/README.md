...
3. **核心数据结构现代化**：引入 `current/versions/migrations` 多版本目录、每表独立 JSON、initial-data 行内结构，为自动生成变更 DDL 铺路。
...
| 版本快照 / 迁移脚本 | 引入 field_id/table_id/schema_id（延迟生成）+ 版本快照 + 结构 diff 引擎 + 迁移脚本生成 MySQL/PostgreSQL 变更 DDL | ✅ 已完成（代码已落地，方案文档已归档删除） |
